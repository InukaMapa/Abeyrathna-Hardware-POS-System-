                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        {selectedSupplier && (
                            <div className="modal-overlay z-[2000] flex items-center justify-center p-6 backdrop-blur-md bg-black/70">
                                <div className="bg-[#1E1E1E] w-full max-w-4xl max-h-[85vh] rounded-[32px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] border border-white/5 overflow-hidden animate-scale-up flex flex-col relative">
                                    ...
                                    <div className="px-8 py-5 grid grid-cols-2 gap-4 bg-[#1E1E1E]">
                                        {[
                                    ...
                                    <div className="flex-1 overflow-y-auto p-8 pt-6 custom-scrollbar bg-[#1E1E1E]">
                                            ...
                                            <div className="space-y-8 animate-fade-in pr-2 max-h-[500px] overflow-y-auto custom-scrollbar">
                                                            ...
                                                            {profileReturns.filter(r => r.resolution_type === 'CREDIT_NOTE').map(note => (
                                                                <div key={note.id} className="p-5 bg-blue-500/5 border border-blue-500/10 rounded-[24px] flex items-center justify-between hover:bg-blue-500/10 transition-all group">
                                                                    <div className="flex items-center gap-4">
                                                                        ...
                                                                        <div>
                                                                            <p className="text-[12px] font-black text-white tracking-widest uppercase">{note.credit_note_number}</p>
                                                                            <p className="text-[10px] text-white/60 font-bold uppercase tracking-tighter mt-1">Origin: {note.return_number}</p>
                                                                        </div>
                                                                    </div>
                                                                    <div className="text-right">
                                                                        <p className="text-sm font-black text-blue-400">Rs. {(parseFloat(note.refund_amount) || 0).toLocaleString()}</p>
                                                                        <p className="text-[9px] text-white/60 uppercase font-black tracking-tighter mt-1">Avail. Credit</p>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                                ...
                                                <div className="space-y-4">
                                                    <div className="flex items-center justify-between mb-4">
                                                        <div>
                                                            <h4 className="text-[13px] font-bold text-white uppercase tracking-[0.3em]">Supplier Ledger</h4>
                                                            <p className="text-[10px] text-white/50 mt-1 uppercase font-bold">Statement of all financial dealings</p>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button className="btn-premium-green-icon" title="Download Ledger">
                                                    ...
                                                    <div className="bg-white/[0.01] border border-white/5 rounded-3xl overflow-hidden">
                                                        <table className="w-full text-left">
                                                            <thead>
                                                                <tr className="border-b border-[#333] bg-[#121212]">
                                                                    <th className="px-6 py-4 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em]">Date</th>
                                                                    <th className="px-6 py-4 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em]">Reference No</th>
                                                                    <th className="px-6 py-4 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em]">Description</th>
                                                                    <th className="px-6 py-4 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em] text-right">Debit / Credit</th>
                                                                    <th className="px-6 py-4 text-[10px] font-bold text-white/60 uppercase tracking-[0.2em] text-center">Status</th>
                                                                </tr>
                                                            </thead>
                                                                ...
                                                                {getLedgerEntries().map((entry, idx) => (
                                                                    <tr key={idx} className="hover:bg-white/[0.02] transition-colors group">
                                                                        <td className="px-6 py-5">
                                                                            <span className="text-[11px] font-bold text-white/90 tracking-tighter">
                                                                                {new Date(entry.date).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                                            </span>
                                                                        </td>
                                                                        <td className="px-6 py-5">
                                                                            <span className={`text-[11px] font-black tracking-tight ${entry.type === 'PAYMENT' ? 'text-[#16A34A]' : 'text-white'}`}>
                                                                                {entry.ref}
                                                                            </span>
                                                                        </td>
                                                                        <td className="px-6 py-5">
                                                                            <div className="flex items-center gap-3">
                                                                                <div className={`p-2 rounded-lg ${entry.type === 'PAYMENT' ? 'bg-[#16A34A]/10 text-[#16A34A]' : 'bg-[#D4AF37]/10 text-[#D4AF37]'}`}>
